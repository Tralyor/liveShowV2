    ...
    <div id="mainBody">
        <form class="form-inline" style="margin-top: 50px;margin-left: 50px">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="classNum" placeholder="课程节次">
            </div>

            <button type="button" class="btn btn-default" id="queryDanmaku">查询</button>
        </form>
    </div>
...
<script>
    ...
    $(document).on("click", "#queryDanmaku", function() {
        var classId = $("#classId").val();
        var classNum= $("#classNum").val();

        $.ajax({
            url: "/danmaku/danmakuData",
            type: "post",
            data: "classId=" + classId + "&classNum=" + classNum,
            dataType: "json",
            success: function(show) {
                ...
                {
                    var str = " <table class=\"table table-striped\"><tr><th>发言人</th><th>姓名</th><th>弹幕内容</th></tr>"
                    if  (show.success && show.data!=null && show.data instanceof  Array) {
                        for  ( var i = 0 ; i <  show.data.length ; i++) {
                            str = str + "<tr><td><div><img class=\"border\" src='"+ show.data[i].userImg+"'"+"style=\"width: 30px;height: 30px;\"></div></td><td>"+show.data[i].userName+"</td><td>"+show.data[i].danmaku.content+"<td></tr>"
                        }
                    }
                    str = str + "</table>"
